...
import React from "react";
import { motion } from "framer-motion";
import { FaGithub, FaLinkedin, FaFacebook, FaTwitter, FaYoutube } from "react-icons/fa";
import { Download, MessageSquare } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
...
export default function Hero() {
  return (
    // 1. Changed to block on mobile / grid on desktop, removed strict min-h calculations to prevent zero-height bugs
    <section 
      id="home" 
      className="block lg:grid lg:grid-cols-2 gap-12 items-center w-full min-h-screen py-16 px-4 text-white"
    >
      {/* LEFT CONTENT COLUMN */}
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        ...
        className="space-y-6 w-full mb-12 lg:mb-0"
      >
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-950/40 border border-purple-500/30 text-xs font-semibold text-purple-300">
        ...
        <div className="flex flex-wrap gap-4 pt-2">
          <button className="p-3 bg-blue-600 hover:bg-blue-700 text-white gap-2 rounded-md flex items-center font-medium">
            <Download size={18} /> Download Resume
          </button>
          <button className="p-3 border border-gray-700 hover:bg-gray-800 text-gray-300 gap-2 rounded-md flex items-center font-medium">
            <MessageSquare size={18} /> Contact Me
          </button>
        </div>
        ...
        <div className="flex items-center gap-4 pt-6">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="https://github.com/tareq-360" className="p-2 rounded-md bg-gray-900 border border-gray-800 text-gray-400 hover:text-blue-400 transition-all">
              <FaGithub size={16} />
            </Link>
            <Link href="https://www.linkedin.com/in/tareq360" className="p-2 rounded-md bg-gray-900 border border-gray-800 text-gray-400 hover:text-blue-400 transition-all">
              <FaLinkedin size={16} />
            </Link>
            <Link href="https://www.facebook.com/share/19P3cCnjmJ/" className="p-2 rounded-md bg-gray-900 border border-gray-800 text-gray-400 hover:text-blue-400 transition-all">
              <FaFacebook size={16} />
            </Link>
            <a href="#" className="p-2 rounded-md bg-gray-900 border border-gray-800 text-gray-400 hover:text-blue-400 transition-all">
              <FaTwitter size={16} />
            </a>
            <Link href="https://www.youtube.com/@the.tech.world.360" className="p-2 rounded-md bg-gray-900 border border-gray-800 text-gray-400 hover:text-blue-400 transition-all">
              <FaYoutube size={16} />
            </Link>
          </div>
        </div>
      ...
      >
        <div className="relative flex justify-center items-center w-72 h-72 sm:w-96 sm:h-96">
          {/* 2. REMOVED: Custom Tailwind arbitrary spin animations that might be breaking or causing layout collapse */}
          ...
          <div className="relative w-48 h-48 sm:w-64 sm:h-64 rounded-full bg-gradient-to-tr from-blue-600/20 to-purple-600/20 flex items-center justify-center overflow-hidden border border-white/10 backdrop-blur-sm">
            <Image 
              src="/Hero-pic.jpg" 
              alt="md tareq hossain" 
              width={230} 
              height={250} 
              priority // 3. Added priority to make sure mobile browsers load it immediately
